...
#(1)
# 捕获脚本的结束
# trap "echo '该脚本执行结束了'" exit      可用于捕获脚本的退出
...
#也是显示在当前终端显示窗口的。与我们后续使用命令的stdout杂在一起会导致显示屏看起来有点乱。
...
#(4)
# nohup可以做到真正后台运行脚本,即使启动该脚本进程的终端窗口关闭了,后台运行的进程也不会关闭(但是后台允许的该进程无法使用ps命令查询到)
