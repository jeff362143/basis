package package2;
/*
继承就是纯抽象的接口,不能有正常的实例的函数，抽象函数前面默认修饰符为 public abstract，
...
3、接口中可以定义一个default方法，该方法子类可以不必实现，只在需要时再重写
4、接口可以定义字段，只是有两个限制：
a.字段修饰必须是 public static final类型,可以省略    b.初始化后必须赋值
 */
public class 接口 implements skill{
    public void sing(){}
    public void jump(){}
    public void rap(){}
...
interface skill{
    ...
    default void basketball(){
        System.out.println("play basketball is a my sport");
    }
        }
