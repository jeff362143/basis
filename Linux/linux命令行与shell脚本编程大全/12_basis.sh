...
# test命令用于在if中进行字符串、数值比较(数值得使用参数...煞笔)、文件比较
# 比较数值。后面有双括号比较表达式。这个别看了😅😅😅😅😅😅😅😅😅😅😅😅😅😅😅😅😅😅
if test 1 -eq 1
then
  echo 'true'
fi
...
# 需要各类比较参数,-d 用于检查是否存在且是一个目录
#directory=/Users/j223hr331/Desktop/git add/linux命令行与shell脚本编程大全  # 目录路径存在空格shell报错
